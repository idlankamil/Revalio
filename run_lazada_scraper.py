...
from services.lazada_scraper import search_lazada, save_lazada_price


# ============================================================
# SELECTED PRODUCTS TO SCRAPE
# Same 20 products as Shopee — is_selected = True only
# ============================================================
...
SELECTED_PRODUCTS = [
    ...
    {"id": 111, "name": "Acer Swift X SFX14-42G-R8Y5",          "brand": "Acer",        "category": "laptops"},
    ...
    {"id": 132, "name": "NuPhy Air75 V3",                   "brand": "NuPhy",       "category": "keyboards"},
    ...
    {"id": 147, "name": "Dell UltraSharp U2723QE 27",       "brand": "Dell",        "category": "monitors"},
]



DELAY_BETWEEN_PRODUCTS = 20  # slightly longer than Shopee — Lazada can be slower
...
def run_all():
    print(f"\n{'='*60}")
    print(f"REVALIO — Lazada Price Scraper (Batch Run)")
    ...
    for i, product in enumerate(SELECTED_PRODUCTS, 1):
        ...
        try:
            results = search_lazada(product["name"], product["brand"], product["category"])

            if results:
                # Median price — avoids cheap accessories at the bottom
                sorted_results = sorted(results, key=lambda x: x["price"])
                best = sorted_results[len(sorted_results) // 2]
                save_lazada_price(product["id"], best)
                success += 1
